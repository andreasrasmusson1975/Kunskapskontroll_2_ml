...
* Supports re-training for final model optimization.  
...
class MnistSVCClassifier(BaseEstimator):
    ...
    def refit_for_validation_set(self,X: np.ndarray,y: np.ndarray) -> 'MnistSVCClassifier':
        """
    ...
    def refit_for_final_model(self,X: np.ndarray,y:np.ndarray) -> 'MnistSVCClassifier':
        """
        Refits the model on the full dataset.
    ...
    def evaluate(self,X: np.ndarray,y:np.ndarray,validation):
        """
        Evaluates the SVM model on the validation set.
        ...
        if validation:
            joblib.dump(eval,'../evaluations/mnist_svc_validation_evaluation.pkl')
        else:
            joblib.dump(eval,'../evaluations/mnist_svc_test_evaluation.pkl')
