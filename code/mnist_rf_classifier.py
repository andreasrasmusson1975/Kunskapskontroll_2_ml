...
class MnistRfClassifier(BaseEstimator):
    ...
    refit_for_validation_set(X, y)
        Re-trains the model for validation set evaluation.
    ...
    def fit(self,X: np.ndarray,y: np.ndarray) -> 'MnistRfClassifier':
        ...
        def tqdm_callback(res):
            ...
            pbar.update(1)
        # Initialize Bayesian search cross validation object
    ...
    def refit_for_validation_set(self,X:np.ndarray,y:np.ndarray) -> 'MnistRfClassifier':
        """
    ...
    def evaluate(self,X: np.ndarray,y:np.ndarray,validation=True):
        """
        Evaluates the Random Forest model on a set.
        ...
        if validation:
            joblib.dump(eval,'../evaluations/mnist_rf_validation_evaluation.pkl')
        else:
            joblib.dump(eval,'../evaluations/mnist_rf_test_evaluation.pkl')
